...
\title{Leveraging crowd to improve data credibility for mobile crowdsensing}
%
%
...
{Shell \MakeLowercase{\textit{et al.}}: Leveraging crowd to improve data credibility for mobile crowdsensing}
...
\begin{IEEEkeywords}
mobile crowdsensing, data credibility, cluster analysis, reputation mechanism.
\end{IEEEkeywords}
...
\begin{enumerate}
\item announced location $L_i$ of $u_i$ is valid if it is the same with its current location.
\item sensory data $S_i$ is valid if it reflects the real physical scenario of the corresponding location $L$.
\end{enumerate}
Based on the validity of $L_i$ and $S_i$, data $d_i$ can be classified into 4 categories showed in Table \ref{false data category}. Note that existed location attestation-based schemes try to improve reliability by identifying data in category B\footnote{Indeed, providing valid sensory data(category B) at invalid location is not feasible as a user is unable to contribute valid data of one location when he is not really there.} and C, ignoring possibly false data in category A, which is more often the case(common) for a malicious user to fabricate.

\begin{table}[h]
\centering
\caption{My caption}
\label{false data category}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|cc|}
\hline
\diagbox{$L_i$}{$d_i$}  & T           & F           \\ \hline
T & Normal data & class A \\
F & class B & class C \\ \hline
\end{tabular}
\end{table}
...
$$
\Re _{\rm{D}}  = ({{{\rm{|f}}(D) - f(\tilde D)|} \over {\min ({\rm{f}}(D),f(\tilde D))}})^{ - 1}
$$
...
%We consider a location as valid if its announced location is the same with its current location. We consider the sensory data as valid if the submitted data reflects the real physical scenario of the corresponding location.
...
\begin{figure}
  \centering
  %\includegraphics[width=.5\textwidth]{fig7.eps}\\
  \includegraphics[width=90mm]{framework.eps}\\
  \caption{Framework of CRDaFi scheme}
  \label{fig:fig1-framework}
\end{figure}
